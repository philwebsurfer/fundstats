---
title: "Presentation - BUGS Example"
output:
  html_document:
    df_print: paged
---
...
_123972 - Ángel Rafael Ortega Ramírez_
...
### Non-Hierarchical BUGS Model

```{bash echo=FALSE}
cat expositionb.txt
```

### Hierarchical BUGS Model
...
The three follow-ups $j$ per child $i$, in log-titre:
...
#-Running code-
#OpenBUGS
simb<-bugs(data,inits,parameters,model.file="expositionb.txt",
              n.iter=5000,n.chains=2,n.burnin=500)
simb
```

### Diagnostic and convergence graphs
...
#-Running code-
#OpenBUGS
sim<-bugs(data,inits,parameters,model.file="exposition.txt",
              n.iter=5000,n.chains=2,n.burnin=500)
# sim
sim
```
...
Graphs for $\omega^2_\alpha$

```{r echo=FALSE}
out<-sim$sims.list
z<-out$omega.alpha.squared
par(mfrow=c(2,2))
plot(z,type="l")
plot(cumsum(z)/(1:length(z)),type="l")
hist(z,freq=FALSE)
...
Graphs for $\omega^2_\beta$

```{r echo=FALSE}
out<-sim$sims.list
z<-out$omega.beta.squared
par(mfrow=c(2,2))
plot(z,type="l")
plot(cumsum(z)/(1:length(z)),type="l")
hist(z,freq=FALSE)
...
Comparison between hierarchical models:

Hierarchical Model                  |||  Non Hierarchical Model              |||
--- | --- | --- | --- | --- | --- |
**_Parameter_** | **_95% interval_** | **_Mean Value_** | **_Mean Value_** | **_95% interval_** | **_Parameter_**
$\mu_\alpha$ | (`r quantile(sim$sims.list$mu.alpha, 0.025, names=F)`, `r quantile(sim$sims.list$mu.alpha, 1-0.025, names=F)`) | `r mean(sim$sims.list$mu.alpha)` | `r mean(simb$sims.list$alpha)` | (`r quantile(simb$sims.list$alpha, 0.025, names=F)`, `r quantile(simb$sims.list$alpha, 1-0.025, names=F)`) | $\alpha$
$\mu_\beta$ | (`r quantile(sim$sims.list$mu.beta, 0.025, names=F)`, `r quantile(sim$sims.list$mu.beta, 1-0.025, names=F)`) | `r mean(sim$sims.list$mu.beta)` | `r mean(simb$sims.list$beta)` | (`r quantile(simb$sims.list$beta, 0.025, names=F)`, `r quantile(simb$sims.list$beta, 1-0.025, names=F)`) |  $\beta$
$\gamma$ | (`r quantile(sim$sims.list$gamma, 0.025, names=F)`, `r quantile(sim$sims.list$gamma, 1-0.025, names=F)`) | `r mean(sim$sims.list$gamma)` | `r mean(simb$sims.list$gamma)` | (`r quantile(simb$sims.list$gamma, 0.025, names=F)`, `r quantile(simb$sims.list$gamma, 1-0.025, names=F)`) | $\gamma$ 
$\sigma^2$ | (`r quantile(sim$sims.list$sigma.squared, 0.025, names=F)`, `r quantile(sim$sims.list$sigma.squared, 1-0.025, names=F)`) | `r mean(sim$sims.list$sigma.squared)` | `r mean(simb$sims.list$sigma.squared)` | (`r quantile(simb$sims.list$sigma.squared, 0.025, names=F)`, `r quantile(simb$sims.list$sigma.squared, 1-0.025, names=F)`) | $\sigma^2$
$\omega^2_\alpha$ | (`r quantile(sim$sims.list$omega.alpha.square, 0.025, names=F)`, `r quantile(sim$sims.list$omega.alpha.square, 1-0.025, names=F)`) | `r mean(sim$sims.list$omega.alpha.squared)` 
$\omega^2_\beta$ | (`r format(quantile(sim$sims.list$omega.beta.squared, 0.025, names=F),scientific=T)`, `r quantile(sim$sims.list$omega.beta.squared, 1-0.025, names=F)`) | `r mean(sim$sims.list$omega.beta.squared)` 
DIC | - | `r mean(sim$DIC)` | `r mean(simb$DIC)`  | -  | DIC


Recalling both models:
...
It is easy to appreciate that both models _capture_ the intercept and slope parameters ($\alpha$/$\mu_\alpha$, $\beta$/$\mu_\beta$ for time and $\gamma$ for titre) are similar in both models, largely due to the linear nature of both. 

What is noteworth is that the variability is greatly reduced in the hierarchical model, about 1/3. This is suggests that the variability can be reduced by allowing child-specific intercepts.

We can remark that the _Deviance Information Criterion_, being a measure of divergence and thereby a comparison between models, show that the hierarchical model is +20% better than the non-hierarchical model, even after the DIC penalizes the less parsimonous model, which is the hierarchical with 94 additional parameters.

## References

* Lunn, D., Jackson, C., Best, N., Thomas, A., & Spiegelhalter, D. (2012). The Bugs Book: A Practical Introduction to Bayesian Analysis (1st ed.). Chapter 10.

* Nieto-Barajas, L. (2019). Notas del Curso de Regresión Avanzada. 
